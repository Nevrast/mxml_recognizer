...
\chapter{Wstęp}
\tab Celem pracy było stworzenie aplikacji pozwalającej na klasyfikację klasycznych utworów muzycznych z wykorzystaniem algorytmów uczenia maszynowego oraz formatu MusicXML.\\
...
<score-partwise version="3.1">
\end{lstlisting}
To jest główny \textit{korzeń} (ang. \textit{root}) dokumentu, element <score-partwise> składa się z części, a każda z części składa się\linebreak z taktów. Atrybut elementu \textit{version} definiuje, którą wersję MusicXML używamy.
...
\item --classify [PATH] - ścieżka do zapisanego modelu, jeśli został podany argument --train użyty zostanie aktualny model\linebreak z pamięci,
...
\tab Celem pracy inżynierskiej było wykonanie aplikacji do automatycznej klasyfikacji utworów muzycznych ze względu na przynależność do danej epoki, cel został wykonany jednak wyniki działania aplikacji nie są idealne i jest na pewno sporo miejsca na poprawę.
\section{Wyniki}
WIP
\section{Wnioski}
WIP
